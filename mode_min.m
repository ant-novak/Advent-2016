...
%   $Revision: 1.1.6.4 $  $Date: 2010/08/23 23:07:41 $
...
for j=1:ncols
    ...
    [minfreq,firstloc] = min(freq);          % find most frequent
    M(j) = v(start(firstloc));               % smallest most frequent
    if dofreq
        F(j) = minfreq;                      % highest frequency
    end
    if docell
        if wassparse
            C{j} = sparse(v(start(freq==minfreq)));
        else
            C{j} = v(start(freq==minfreq));  % all most frequent
        end
    end
end
