...
def move_elevator(par, avec, state):
	...
	if check([x + par*y for x, y in zip(state[0:10], avec)]):
		ok = True
		state2 = [x + par*y for x, y in zip(state, avec)]
		state2.extend([state[10]+par])
	return [state2,ok];
...
for k in range(35):
	...
	keep_states = [s for (p,s) in sorted(zip(dists,new_states))][0:1000]


	
	if [4,4,4,4,4,4,4,4,4,4,4] in new_states:
		print(str(k+1) + ' steps is OK')
